...
const Item = ({ images, user, title, star, description, attributes, address, id }) => {
    ...
    return (
        <div className='w-full flex border-t border-orange-600 py-4'>
            ...
            >
                {/* Kiểm tra images trước khi truy cập length hoặc filter */}
                {images && images.length > 0 && images.filter((img, index) => indexs.includes(index)).map((img, index) => (
                    <img key={index} src={img} alt="preview" className='w-[47%] h-[120px] object-cover' />
                ))}
                <span className='bg-overlay-70 text-white px-2 rounded-md absolute left-1 bottom-4'>{`${images ? images.length : 0} ảnh`}</span>
                <span
